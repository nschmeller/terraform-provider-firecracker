// firecrackermanager implements a service to create and delete firecracker VMs
// Adapted from https://gist.github.com/jvns/9b274f24cfa1db7abecd0d32483666a3
package main
...
// 	err := io.WriteFile(metaDataPath, []byte("instance-id: i-litchi12345"), 0644)
// 	if err != nil {
...
func (opts *options) createVMM(ctx context.Context) (*RunningFirecracker, error) {
	...
	if err := exec.Command("sysctl", "-w", fmt.Sprintf("net.ipv4.conf.%s.proxy_arp=1", opts.TapDev)).Run(); err != nil {
		...
		return nil, fmt.Errorf("failed doing ipv4 sysctl: %s", err)
	}
	if err := exec.Command("sysctl", "-w", fmt.Sprintf("net.ipv6.conf.%s.disable_ipv6=1", opts.TapDev)).Run(); err != nil {
		if vmmCancel != nil {
			vmmCancel()
		}
		return nil, fmt.Errorf("failed doing ipv6 sysctl: %s", err)
	}
...
func copyImage(src string) (string, error) {
	...
	destination, err := os.CreateTemp("", "image")
	if err != nil {
		return "", err
	}
